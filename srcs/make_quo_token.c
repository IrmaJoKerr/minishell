...
/*   Updated: 2025/05/22 09:42:53 by bleow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int merge_quoted_token(char *input, char *content, t_vars *vars)
{
    ...
    fprintf(stderr, "DEBUG-MERGE: Entering merge_quoted_token with content '%s'\n", content);
    fprintf(stderr, "DEBUG-MERGE: Adjacency state: {%d,%d}\n", 
            vars->adj_state[0], vars->adj_state[1]);
    
    if (!vars->adj_state[0])
    {
        fprintf(stderr, "DEBUG-MERGE: No left adjacency, cannot merge\n");
        return (0);
    }
    ...
    if (!dummy_token)
    {
        fprintf(stderr, "DEBUG-MERGE: Failed to allocate dummy token\n");
        return (0);
    }
    
    fprintf(stderr, "DEBUG-MERGE: Calling handle_tok_join to merge with previous token\n");
    fprintf(stderr, "DEBUG-MERGE: Current token: %p\n", (void*)vars->current);
    if (vars->current && vars->current->args)
        fprintf(stderr, "DEBUG-MERGE: Current token value: '%s'\n", vars->current->args[0]);
        
    join_success = handle_tok_join(input, vars, content, dummy_token);
    
    if (!join_success)
    {
        fprintf(stderr, "DEBUG-MERGE: Join failed, freeing dummy token\n");
        free(dummy_token);
    }
    else
    {
        fprintf(stderr, "DEBUG-MERGE: Join succeeded\n");
        if (vars->adj_state[1])
        {
            fprintf(stderr, "DEBUG-MERGE: Processing right adjacency\n");
            process_right_adj(input, vars);
        }
        process_adj(NULL, vars);
    }
    
    fprintf(stderr, "DEBUG-MERGE: Returning %d\n", join_success);
    return (join_success);
}
