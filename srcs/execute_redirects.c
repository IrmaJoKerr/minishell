...
/*   Updated: 2025/05/18 14:53:14 by bleow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int setup_in_redir(t_node *node, t_vars *vars)
{
    ...
    {
        fprintf(stderr, "DEBUG-IN-REDIR: Permission check failed for '%s'\n", file);
        vars->error_code = 127; // Special error code for redirection failures
        // Remove end_pipe_processes to avoid killing the entire pipeline
        return (0);
    }
...
int proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
{
    ...
    while (current_node && is_redirection(current_node->type))
    {
        ...
        if (!setup_redirection(current_node, vars))
        {
            fprintf(stderr, "DEBUG-REDIR-CHAIN: Redirection failed but continuing process\n");
            // KEY FIX: Don't reset file descriptors on error in pipes
            // This allows the right side of a pipe to continue even if left fails
            if (vars->pipes->pipe_root) {
                return (0); // Return error but don't clean up redirections
            } else {
                reset_redirect_fds(vars);
                return (0);
            }
