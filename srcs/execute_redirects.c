/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   execute_redirects.c                                :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: bleow <bleow@student.42kl.edu.my>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/25 22:39:34 by bleow             #+#    #+#             */
/*   Updated: 2025/05/04 21:19:04 by bleow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

/*
Handles redirection setup for input files.
- Opens file for reading.
- Redirects stdin to read from the file.
- Properly handles and reports errors.
Returns:
- 1 on success.
- 0 on failure.
Works with setup_redirection().
*/
int	setup_in_redir(t_node *node, t_vars *vars)
{
	char	*file;

	if (!node->right || !node->right->args || !node->right->args[0])
		return (0);
	file = node->right->args[0];
	if (!chk_permissions(file, O_RDONLY, vars))
	{
		end_pipe_processes(vars);
		return (0);
	}
	vars->pipes->redirection_fd = open(file, O_RDONLY);
	if (vars->pipes->redirection_fd == -1)
	{
		not_found_error(file, vars);
		end_pipe_processes(vars);
		return (0);
	}
	if (dup2(vars->pipes->redirection_fd, STDIN_FILENO) == -1)
	{
		close(vars->pipes->redirection_fd);
		return (0);
	}
	return (1);
}

/*
Handles redirection setup for output files.
- Opens file for writing in truncate or append mode.
- Redirects stdout to the opened file.
- Properly handles and reports errors.
Returns:
- 1 on success.
- 0 on failure.
Works with setup_redirection().
*/
int	setup_out_redir(t_node *node, t_vars *vars)
{
	char	*file;
	int		flags;

	if (!node->right || !node->right->args || !node->right->args[0])
		return (0);
	flags = O_WRONLY | O_CREAT;
	file = node->right->args[0];
	if (vars->pipes->out_mode == OUT_MODE_APPEND)
		flags |= O_APPEND;
	else
		flags |= O_TRUNC;
	if (!chk_permissions(file, flags, vars))
		return (0);
	vars->pipes->redirection_fd = open(file, flags, 0644);
	if (vars->pipes->redirection_fd == -1)
	{
		shell_error(file, ERR_PERMISSIONS, vars);
    	// if (DEBUG_ERROR) //DEBUG PRINT
		// {
        // 	fprintf(stderr, "[DEBUG] setup_out_redir() cannot open: %s, error_code=%d\n", 
        //         file, vars->error_code); //DEBUG PRINT
		// }
		return (0);
	}
	if (dup2(vars->pipes->redirection_fd, STDOUT_FILENO) == -1)
	{
		close(vars->pipes->redirection_fd);
		return (0);
	}
	return (1);
}

/*
Sets up heredoc redirection handling.
- Checks if heredoc content is ready.
- Initiates interactive mode if end delimiter not found.
- Handles the actual heredoc setup via handle_heredoc().
Returns:
- 1 on success
- 0 on failure
Works with redir_mode_setup().
*/
int	setup_heredoc_redir(t_node *node, t_vars *vars)
{
	int	result;

	result = 1;
	if (!vars->hd_text_ready)
	{
		if (!interactive_hd_mode(vars))
			return (0);
	}
	result = handle_heredoc(node, vars);
	return (result);
}

/*
Processes a chain of redirections for a command node.
- Sets up each redirection in the chain.
- Navigates through connected redirections.
- Handles errors and cleanup if a redirection fails.
Returns:
- 1 on success (all redirections processed)
- 0 on failure (at least one redirection failed)
Works with exec_redirect_cmd().
*/
// int	proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
// {
// 	t_node	*current_node;
// 	t_node	*next_redir;

// 	current_node = start_node;
// 	while (current_node && is_redirection(current_node->type))
// 	{
// 		vars->pipes->current_redirect = current_node;
// 		if (!setup_redirection(current_node, vars))
// 		{
// 			reset_redirect_fds(vars);
// 			return (0);
// 		}
// 		if (current_node->redir)
// 			current_node = current_node->redir;
// 		else
// 		{
// 			next_redir = get_next_redir(current_node, cmd_node);
// 			if (next_redir)
// 				current_node = next_redir;
// 			else
// 				break ;
// 		}
// 	}
// 	return (1);
// }
// int proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
// {
//     t_node *current_node;
//     t_node *next_redir;
//     char **original_args = NULL;
//     int arg_count = 0;
    
//     fprintf(stderr, "[DEBUG-CHAIN] Processing redirection chain\n");
    
//     // Debug print command node before processing
//     if (cmd_node) {
//         fprintf(stderr, "[DEBUG-CHAIN] Command node before redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Save original arguments before any redirection processing
//     if (cmd_node && cmd_node->args) {
//         arg_count = 0;
//         while (cmd_node->args[arg_count]) {
//             arg_count++;
//         }
        
//         // Create deep copy of original arguments
//         original_args = ft_calloc(arg_count + 1, sizeof(char *));
//         if (original_args) {
//             int i = 0;
//             while (i < arg_count) {
//                 original_args[i] = ft_strdup(cmd_node->args[i]);
//                 if (!original_args[i]) {
//                     // Handle memory allocation failure
//                     int j = 0;
//                     while (j < i) {
//                         free(original_args[j]);
//                         j++;
//                     }
//                     free(original_args);
//                     original_args = NULL;
//                     break;
//                 }
//                 i++;
//             }
//             if (original_args)
//                 original_args[arg_count] = NULL;
//         }
//     }

//     // Process all redirections in the chain
//     current_node = start_node;
//     while (current_node && is_redirection(current_node->type))
//     {
//         fprintf(stderr, "[DEBUG-CHAIN] Processing redirection type: %s\n", 
//             get_token_str(current_node->type));
            
//         vars->pipes->current_redirect = current_node;
//         if (!setup_redirection(current_node, vars))
//         {
//             fprintf(stderr, "[DEBUG-CHAIN] Redirection setup failed\n");
//             reset_redirect_fds(vars);
//             if (original_args) {
//                 int i = 0;
//                 while (i < arg_count) {
//                     free(original_args[i]);
//                     i++;
//                 }
//                 free(original_args);
//             }
//             return (0);
//         }
        
//         if (current_node->redir)
//             current_node = current_node->redir;
//         else
//         {
//             next_redir = get_next_redir(current_node, cmd_node);
//             if (next_redir)
//                 current_node = next_redir;
//             else
//                 break;
//         }
//     }
    
//     // After all redirections processed, restore original arguments if needed
//     if (original_args && cmd_node && cmd_node->type == TYPE_CMD) {
//         fprintf(stderr, "[DEBUG-CHAIN] Checking if args need restoration\n");
        
//         // Check if arguments were lost or changed
//         int current_arg_count = 0;
//         if (cmd_node->args) {
//             while (cmd_node->args[current_arg_count])
//                 current_arg_count++;
//         }
        
//         fprintf(stderr, "[DEBUG-CHAIN] Original arg count: %d, Current arg count: %d\n", 
//             arg_count, current_arg_count);
        
//         // If arguments were lost or changed, restore them
//         if (current_arg_count != arg_count) {
//             fprintf(stderr, "[DEBUG-CHAIN] Args changed, restoring original args\n");
            
//             // Free current args if they exist
//             if (cmd_node->args) {
//                 int i = 0;
//                 while (cmd_node->args[i]) {
//                     free(cmd_node->args[i]);
//                     i++;
//                 }
//                 free(cmd_node->args);
//             }
            
//             // Create new args array
//             cmd_node->args = ft_calloc(arg_count + 1, sizeof(char *));
//             if (cmd_node->args) {
//                 // Copy back the original arguments
//                 int i = 0;
//                 while (i < arg_count) {
//                     cmd_node->args[i] = ft_strdup(original_args[i]);
//                     i++;
//                 }
//                 cmd_node->args[arg_count] = NULL;
//             }
//         }
        
//         // Debug print command node after processing
//         fprintf(stderr, "[DEBUG-CHAIN] Command node after redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Clean up original_args
//     if (original_args) {
//         int i = 0;
//         while (i < arg_count) {
//             free(original_args[i]);
//             i++;
//         }
//         free(original_args);
//     }
    
//     fprintf(stderr, "[DEBUG-CHAIN] Redirection chain processed successfully\n");
//     return (1);
// }
// int proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
// {
//     t_node *current_node;
//     t_node *next_redir;
//     char **original_args = NULL;
//     int arg_count = 0;
    
//     fprintf(stderr, "[DEBUG-CHAIN] Processing redirection chain\n");
    
//     // Debug print command node before processing
//     if (cmd_node) {
//         fprintf(stderr, "[DEBUG-CHAIN] Command node before redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Save original arguments before any redirection processing
//     if (cmd_node && cmd_node->args) {
//         arg_count = 0;
//         while (cmd_node->args[arg_count]) {
//             arg_count++;
//         }
        
//         // Create deep copy of original arguments
//         original_args = ft_calloc(arg_count + 1, sizeof(char *));
//         if (original_args) {
//             int i = 0;
//             while (i < arg_count) {
//                 original_args[i] = ft_strdup(cmd_node->args[i]);
//                 if (!original_args[i]) {
//                     // Handle memory allocation failure
//                     int j = 0;
//                     while (j < i) {
//                         free(original_args[j]);
//                         j++;
//                     }
//                     free(original_args);
//                     original_args = NULL;
//                     break;
//                 }
//                 i++;
//             }
//             if (original_args)
//                 original_args[arg_count] = NULL;
//         }
//     }

//     // Process all redirections in the chain
//     current_node = start_node;
//     while (current_node && is_redirection(current_node->type))
//     {
//         fprintf(stderr, "[DEBUG-CHAIN] Processing redirection type: %s\n", 
//             get_token_str(current_node->type));
            
//         vars->pipes->current_redirect = current_node;
//         if (!setup_redirection(current_node, vars))
//         {
//             fprintf(stderr, "[DEBUG-CHAIN] Redirection setup failed\n");
//             reset_redirect_fds(vars);
//             if (original_args) {
//                 int i = 0;
//                 while (i < arg_count) {
//                     free(original_args[i]);
//                     i++;
//                 }
//                 free(original_args);
//             }
//             return (0);
//         }
        
//         if (current_node->redir)
//             current_node = current_node->redir;
//         else
//         {
//             next_redir = get_next_redir(current_node, cmd_node);
//             if (next_redir)
//                 current_node = next_redir;
//             else
//                 break;
//         }
//     }
    
//     // After all redirections processed, restore original arguments if needed
//     if (original_args && cmd_node && cmd_node->type == TYPE_CMD) {
//         fprintf(stderr, "[DEBUG-CHAIN] Checking if args need restoration\n");
        
//         // Check if arguments were lost or changed
//         int current_arg_count = 0;
//         if (cmd_node->args) {
//             while (cmd_node->args[current_arg_count])
//                 current_arg_count++;
//         }
        
//         fprintf(stderr, "[DEBUG-CHAIN] Original arg count: %d, Current arg count: %d\n", 
//             arg_count, current_arg_count);
        
//         // If arguments were lost or changed, restore them
//         if (current_arg_count != arg_count) {
//             fprintf(stderr, "[DEBUG-CHAIN] Args changed, restoring original args\n");
            
//             // Free current args if they exist
//             if (cmd_node->args) {
//                 int i = 0;
//                 while (cmd_node->args[i]) {
//                     free(cmd_node->args[i]);
//                     i++;
//                 }
//                 free(cmd_node->args);
//             }
            
//             // Create new args array
//             cmd_node->args = ft_calloc(arg_count + 1, sizeof(char *));
//             if (cmd_node->args) {
//                 // Copy back the original arguments
//                 int i = 0;
//                 while (i < arg_count) {
//                     cmd_node->args[i] = ft_strdup(original_args[i]);
//                     i++;
//                 }
//                 cmd_node->args[arg_count] = NULL;
//             }
//         }
        
//         // Debug print command node after processing
//         fprintf(stderr, "[DEBUG-CHAIN] Command node after redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Clean up original_args
//     if (original_args) {
//         int i = 0;
//         while (i < arg_count) {
//             free(original_args[i]);
//             i++;
//         }
//         free(original_args);
//     }
    
//     fprintf(stderr, "[DEBUG-CHAIN] Redirection chain processed successfully\n");
//     return (1);
// }
// int proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
// {
//     t_node *current_node;
//     t_node *next_redir;
//     char **original_args = NULL;
//     int arg_count = 0;
    
//     fprintf(stderr, "[DEBUG-CHAIN] Processing redirection chain\n");
    
//     // Debug print command node before processing
//     if (cmd_node) {
//         fprintf(stderr, "[DEBUG-CHAIN] Command node before redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Save original arguments before any redirection processing
//     if (cmd_node && cmd_node->args) {
//         arg_count = 0;
//         while (cmd_node->args[arg_count]) {
//             arg_count++;
//         }
        
//         // Create deep copy of original arguments
//         original_args = ft_calloc(arg_count + 1, sizeof(char *));
//         if (original_args) {
//             int i = 0;
//             while (i < arg_count) {
//                 original_args[i] = ft_strdup(cmd_node->args[i]);
//                 if (!original_args[i]) {
//                     // Handle memory allocation failure
//                     int j = 0;
//                     while (j < i) {
//                         free(original_args[j]);
//                         j++;
//                     }
//                     free(original_args);
//                     original_args = NULL;
//                     break;
//                 }
//                 i++;
//             }
//             if (original_args)
//                 original_args[arg_count] = NULL;
//         }
//     }

//     // Process all redirections in the chain
//     current_node = start_node;
//     while (current_node && is_redirection(current_node->type))
//     {
//         fprintf(stderr, "[DEBUG-CHAIN] Processing redirection type: %s\n", 
//             get_token_str(current_node->type));
            
//         vars->pipes->current_redirect = current_node;
//         if (!setup_redirection(current_node, vars))
//         {
//             fprintf(stderr, "[DEBUG-CHAIN] Redirection setup failed\n");
//             reset_redirect_fds(vars);
//             if (original_args) {
//                 int i = 0;
//                 while (i < arg_count) {
//                     free(original_args[i]);
//                     i++;
//                 }
//                 free(original_args);
//             }
//             return (0);
//         }
        
//         if (current_node->redir)
//             current_node = current_node->redir;
//         else
//         {
//             next_redir = get_next_redir(current_node, cmd_node);
//             if (next_redir)
//                 current_node = next_redir;
//             else
//                 break;
//         }
//     }
    
//     // After all redirections processed, restore original arguments if needed
//     if (original_args && cmd_node && cmd_node->type == TYPE_CMD) {
//         fprintf(stderr, "[DEBUG-CHAIN] Checking if args need restoration\n");
        
//         // Check if arguments were lost or changed
//         int current_arg_count = 0;
//         if (cmd_node->args) {
//             while (cmd_node->args[current_arg_count])
//                 current_arg_count++;
//         }
        
//         fprintf(stderr, "[DEBUG-CHAIN] Original arg count: %d, Current arg count: %d\n", 
//             arg_count, current_arg_count);
        
//         // If arguments were lost or changed, restore them
//         if (current_arg_count != arg_count) {
//             fprintf(stderr, "[DEBUG-CHAIN] Args changed, restoring original args\n");
            
//             // Free current args if they exist
//             if (cmd_node->args) {
//                 int i = 0;
//                 while (cmd_node->args[i]) {
//                     free(cmd_node->args[i]);
//                     i++;
//                 }
//                 free(cmd_node->args);
//             }
            
//             // Create new args array
//             cmd_node->args = ft_calloc(arg_count + 1, sizeof(char *));
//             if (cmd_node->args) {
//                 // Copy back the original arguments
//                 int i = 0;
//                 while (i < arg_count) {
//                     cmd_node->args[i] = ft_strdup(original_args[i]);
//                     i++;
//                 }
//                 cmd_node->args[arg_count] = NULL;
//             }
//         }
        
//         // Debug print command node after processing
//         fprintf(stderr, "[DEBUG-CHAIN] Command node after redirection:\n");
//         print_node_content(stderr, cmd_node);
//         fprintf(stderr, "\n");
//     }
    
//     // Clean up original_args
//     if (original_args) {
//         int i = 0;
//         while (i < arg_count) {
//             free(original_args[i]);
//             i++;
//         }
//         free(original_args);
//     }
    
//     fprintf(stderr, "[DEBUG-CHAIN] Redirection chain processed successfully\n");
//     return (1);
// }
int proc_redir_chain(t_node *start_node, t_node *cmd_node, t_vars *vars)
{
    t_node *current_node;
    t_node *next_redir;
    char **original_args = NULL;
    int arg_count = 0;
    
    fprintf(stderr, "[DEBUG-CHAIN] Processing redirection chain\n");
    
    // Debug print command node before processing
    if (cmd_node) {
        fprintf(stderr, "[DEBUG-CHAIN] Command node before redirection:\n");
        print_node_content(stderr, cmd_node);
        fprintf(stderr, "\n");
    }
    
    // Save original arguments before any redirection processing
    if (cmd_node && cmd_node->args) {
        arg_count = 0;
        while (cmd_node->args[arg_count]) {
            arg_count++;
        }
        
        // Create deep copy of original arguments
        original_args = ft_calloc(arg_count + 1, sizeof(char *));
        if (original_args) {
            int i = 0;
            while (i < arg_count) {
                original_args[i] = ft_strdup(cmd_node->args[i]);
                if (!original_args[i]) {
                    // Handle memory allocation failure
                    int j = 0;
                    while (j < i) {
                        free(original_args[j]);
                        j++;
                    }
                    free(original_args);
                    original_args = NULL;
                    break;
                }
                i++;
            }
            if (original_args)
                original_args[arg_count] = NULL;
        }
    }

    // Process all redirections in the chain
    current_node = start_node;
    while (current_node && is_redirection(current_node->type))
    {
        fprintf(stderr, "[DEBUG-CHAIN] Processing redirection type: %s\n", 
            get_token_str(current_node->type));
            
        vars->pipes->current_redirect = current_node;
        if (!setup_redirection(current_node, vars))
        {
            fprintf(stderr, "[DEBUG-CHAIN] Redirection setup failed\n");
            reset_redirect_fds(vars);
            if (original_args) {
                int i = 0;
                while (i < arg_count) {
                    free(original_args[i]);
                    i++;
                }
                free(original_args);
            }
            return (0);
        }
        
        if (current_node->redir)
            current_node = current_node->redir;
        else
        {
            next_redir = get_next_redir(current_node, cmd_node);
            if (next_redir)
                current_node = next_redir;
            else
                break;
        }
    }
    
    // After all redirections processed, restore original arguments if needed
    if (original_args && cmd_node && cmd_node->type == TYPE_CMD) {
        fprintf(stderr, "[DEBUG-CHAIN] Checking if args need restoration\n");
        
        // Check if arguments were lost or changed
        int current_arg_count = 0;
        if (cmd_node->args) {
            while (cmd_node->args[current_arg_count])
                current_arg_count++;
        }
        
        fprintf(stderr, "[DEBUG-CHAIN] Original arg count: %d, Current arg count: %d\n", 
            arg_count, current_arg_count);
        
        // If arguments were lost or changed, restore them
        if (current_arg_count != arg_count) {
            fprintf(stderr, "[DEBUG-CHAIN] Args changed, restoring original args\n");
            
            // Free current args if they exist
            if (cmd_node->args) {
                int i = 0;
                while (cmd_node->args[i]) {
                    free(cmd_node->args[i]);
                    i++;
                }
                free(cmd_node->args);
            }
            
            // Create new args array
            cmd_node->args = ft_calloc(arg_count + 1, sizeof(char *));
            if (cmd_node->args) {
                // Copy back the original arguments
                int i = 0;
                while (i < arg_count) {
                    cmd_node->args[i] = ft_strdup(original_args[i]);
                    i++;
                }
                cmd_node->args[arg_count] = NULL;
            }
        }
        
        // Debug print command node after processing
        fprintf(stderr, "[DEBUG-CHAIN] Command node after redirection:\n");
        print_node_content(stderr, cmd_node);
        fprintf(stderr, "\n");
    }
    
    // Clean up original_args
    if (original_args) {
        int i = 0;
        while (i < arg_count) {
            free(original_args[i]);
            i++;
        }
        free(original_args);
    }
    
    fprintf(stderr, "[DEBUG-CHAIN] Redirection chain processed successfully\n");
    return (1);
}
