...
/*   Updated: 2025/04/29 16:48:52 by bleow            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char *debug_env = getenv("MINISHELL_DEBUG");
    if (!debug_env)
        return;
    ...
    // Environmental variable MINISHELL_DEBUG=none can disable all debugging
...
void print_ast_node(FILE *fp, t_node *node, int indent_level)
{
    ...
    // Print redirection chain if exists
    if (node->redir)
    {
        for (i = 0; i < indent_level; i++)
            fprintf(fp, "  ");
